...
public class ProductService {
	...
	public Map<String, Object> getMainPageProductList(long pageNo) {
		
		//1. 페이징을 위한 기본 데이터 계산
		int totalCount = productDao.getMainPageTotalCount();
		if(totalCount==0) {
			return null;
		}
		int pageCount = (int)Math.ceil( (double)totalCount / SHOW_SIZE );
		...
		//4. 리스트 가져오기
		List<ProductVo> products = productDao.getMainPageList(pageNo, SHOW_SIZE );
